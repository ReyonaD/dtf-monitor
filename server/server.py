...
class AuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if is_valid_session(token):
            return await call_next(request)
        # API calls get 401 JSON; browser pages get redirect to login
...
async def start_job(job_id: int, req: PrintActionRequest):
    ...
    if prev_printing_jobs:
        ...
        for pj in prev_printing_jobs:
            ...
            await _match_and_complete_customer_file(pj)

    # Update customer file status to 'printing' for the new job (old flow)
    ...
    # Match by #CODE in filename — mark customer file as 'printing'
    if job:
        cf = match_customer_file_by_code(job.get("filename", ""))
        if cf:
            update_customer_file_status(cf["id"], "printing")
            await customer_manager.send_to_customer(cf["customer_id"], {
                "type": "file_update", "file_id": cf["id"], "status": "printing"
            })
...
async def complete_job_endpoint(job_id: int):
    ...
    if job:
        ...
        # Match by #CODE in filename (new flow)
        for j in jobs_to_update:
            await _match_and_complete_customer_file(j)
...
async def _handle_customer_file_completion(job: dict):
    """If the completed job is linked to a customer file (old flow), update status and deduct credit."""
...
async def _match_and_complete_customer_file(job: dict):
    """Check if a job's filename contains a #CODE matching a customer file.
    If so, mark the customer file as completed and deduct credit."""
    filename = job.get("filename", "")
    cf = match_customer_file_by_code(filename)
    if not cf:
        return
    update_customer_file_status(cf["id"], "completed")
    inches = cf["print_inches"] * cf.get("copies", 1)
    deduct_credit(cf["customer_id"], inches, reference_id=str(job.get("id", "")))
    balance = get_customer_balance(cf["customer_id"])
    await customer_manager.send_to_customer(cf["customer_id"], {
...
async def report_details(machine_name: str = Query(...), day: str = Query(...)):
    return get_report_details(machine_name, day)
...
async def store_cell_details(
    ...
    return get_store_cell_details(store, machine_type, warehouse, start, end)


@app.get("/api/reports/unrecognized")
async def unrecognized_files(start: str = Query(...), end: str = Query(...)):
    return get_unrecognized_files(start, end)


@app.put("/api/reports/store-override")
async def store_override(request: Request):
    body = await request.json()
    filename = body.get("filename", "")
    store_code = body.get("store_code", "")
    if not filename or not store_code:
        return JSONResponse({"error": "filename and store_code required"}, status_code=400)
    set_store_override(filename, store_code)
    return {"status": "ok"}


@app.get("/api/store-codes")
async def list_store_codes():
    return sorted(KNOWN_STORE_CODES)
...
@app.put("/api/machines/{machine_id}/type")
async def set_machine_type(machine_id: str, req: MachineTypeRequest):
    update_machine_type(machine_id, req.machine_type)
    state = build_dashboard_state()
    await manager.broadcast({"type": "state_update", "machines": state})
    return {"status": "ok"}
...
async def assign_file_endpoint(file_id: str, req: AssignFileRequest):
    ...
    # Notify customer
    await customer_manager.send_to_customer(cf["customer_id"], {
        "type": "file_update", "file_id": file_id, "status": "assigned"
    })
    ...
    return {"status": "ok", "warning": warning, "file_id": file_id}
