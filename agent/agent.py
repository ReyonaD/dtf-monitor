...
class AgentApp:
    ...
    def heartbeat_loop(self):
        ...
        while self.running:
            ...
            # Fetch history every 5th cycle (~40s) to reduce load
            if cycle % 5 == 0:
                self.fetch_history()
            cycle += 1
            try:
    ...
    def download_customer_file(self, customer_file_id, original_filename="file"):
        """Download a customer file from the server with Save As dialog."""
        name, ext = os.path.splitext(original_filename)
        save_path = filedialog.asksaveasfilename(
            title="Save customer file",
    ...
    def build_gui(self):
        ...
        self.tab_cfiles_btn = tk.Label(tab_bar, text="Customer Files", font=("Segoe UI", 11),
                                        fg=C["text3"], bg=C["surface"], padx=12, pady=4, cursor="hand2")
        self.tab_cfiles_btn.pack(side="left")
        self.tab_cfiles_btn.bind("<Button-1>", lambda e: self.switch_tab("customer_files"))
    ...
    def switch_tab(self, tab):
        self.active_tab = tab
        for btn, name in [(self.tab_queue_btn, "queue"),
                          (self.tab_history_btn, "history"),
                          (self.tab_cfiles_btn, "customer_files")]:
            if name == tab:
                btn.config(fg=C["accent"], font=("Segoe UI", 11, "bold"))
            else:
                btn.config(fg=C["text3"], font=("Segoe UI", 11))
        self.refresh_ui()
    ...
    def refresh_ui(self):
        ...
        # Customer Files tab
        if self.active_tab == "customer_files":
            self.nest_bar.pack_forget()
            self._render_customer_files_tab()
            return
    ...
    def _build_queue_row(self, job, num):
        ...
        if job.get("customer_file_id") and job.get("id") is not None:
            dl_btn = tk.Button(row, text="\u2B07", font=("Segoe UI", 14),
                               bg="#7c3aed", fg="white",
                               activebackground="#6d28d9", activeforeground="white",
                               padx=8, pady=4, relief="flat", cursor="hand2",
                               command=lambda jid=job["id"], cfid=job["customer_file_id"], fn=job["filename"]: self.download_customer_file(cfid, fn))
    ...
    def _render_customer_files_tab(self):
        """Render customer files assigned to this machine."""
        files = self.customer_files

        if self.search_query:
            files = [f for f in files if self.search_query in f.get("original_filename", "").lower()]

        if not files:
            empty_frame = tk.Frame(self.list_frame, bg=C["bg"], pady=40)
            empty_frame.pack(fill="x")
            msg = "No matching files" if self.search_query else "No customer files assigned"
            tk.Label(empty_frame, text=msg,
                     font=("Segoe UI", 12), fg=C["text3"], bg=C["bg"]).pack()
            return

        self._section_label(f"CUSTOMER FILES  ({len(files)} files)", "#FF7A35")
        ...
        for i, cf in enumerate(files):
            ...
            tk.Label(info, text=cf.get("original_filename", ""),
                     font=("Segoe UI", 12), fg=C["text"], bg=bg,
                     anchor="w", wraplength=300).pack(anchor="w")

            customer_name = cf.get("customer_name", "Unknown")
            copies = cf.get("copies", 1)
            inches = cf.get("print_inches", 0)
            inch_str = f"{inches:.1f} in" if copies <= 1 else f"{inches:.1f} in x{copies}"
            meta = f"{customer_name}  |  {inch_str}  |  {cf.get('status', '')}"
            tk.Label(info, text=meta, font=("Consolas", 9),
                     fg=C["text3"], bg=bg, anchor="w").pack(anchor="w", pady=(2, 0))

            # DL button
            tk.Button(row, text="\u2B07", font=("Segoe UI", 14),
                      bg="#7c3aed", fg="white",
                      activebackground="#6d28d9", activeforeground="white",
                      padx=14, pady=4, relief="flat", cursor="hand2",
                      command=lambda fid=cf["id"], fn=cf.get("original_filename", "file"): self.download_customer_file(fid, fn)
